...
fn update_account(
    ...
    requests: &Mutex<Vec<i32>>,
    requests_fulfilled: &AtomicBool,
) -> () {
...
fn main() {
    ...
    let act = Arc::new(Account {
        balance: Mutex::new(300),
    });
    ...
    for _ in 0..6 {
        ...
        let worker_thread =
            thread::spawn(move || update_account(&*act_ref, &*requests_ref, &*req_fulfilled_ref));
